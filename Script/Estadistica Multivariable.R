...
ggsave("mi_propia_grafica.pdf",device="pdf")##Guarda la grafica en PFD
...
sturges = as.integer(1+log2(150)) ## formula de sturges
...
iris_ordenado %>% filter(Medida == "Sepal.Length")### Esto significa filtrar
...
iris_ordenado %>% filter(Medida=="Sepal.Length") %>%
  ...
  geom_histogram(mapping=aes(x=Valor),
                 fill="red",
                 color="darkblue") +
  labs(title = "Mi histograma Fltrado por Sepal Lengenth",
       subtitle = "Basado en Iris") +
  theme(
    panel.background = element_rect("grey"), 
    panel.grid = element_blank(),
